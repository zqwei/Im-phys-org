$.get( '/data/senddata/', function(data, status){
  if (status === 'success'){
    ...
    html = '<h3>'+title+'</h3>';
    // html += '<span class="ui blue label">'+data_info['label']+'</span>';
    var content = '<div class="ui accordion">'+data_info['description']+'<div class="title"><i class="dropdown icon"></i> Detailed task description </div> <div class="content">'+data_info['description_detail']+'<br><br><img src="'+data_info['description_images']+'" height="150" class="ui centered image"><br><br></div></div></div>';
...
//       html_ += '<div class="ui accordion"><div class="title">  Task description <i class="dropdown icon"></i> </div> <div class="content">'+this['Description']+'<br></div></div>';
      html_ += '<span class="ui small gray label">Sample size</span>' + 'x animals';
      html_ += 'x sessions';
      html_ += 'x neurons' + '<br>';
